...
function scatterPlotForGross(countryName, movieData){
    ...
    let ratings = Array.from(new Set(movieData.map(d => d.rating)));
    // console.log(ratings)
    ...
    group.selectAll("legends")
        ...
        .join('circle')
        .attr("id", d => d)
        .attr('cx', width - margin.right - inset - 125)
        .attr('cy', (d, i) => (i * 10) + 50)
        .attr("fill", (d, i) => colorScale(d))
        .attr("r", 4)
        .on('mouseover', (d, i) => {
            // console.log('mouse over', i)
            d3.selectAll('.grossCircle').attr('opacity', 0.2)
    ...
    group.selectAll('legendText')
        .data(ratings)
        .join('text')
        .attr('x', width - margin.right - inset - 125 + 10)
        .attr('y', (d, i) => (i * 10) + 50)
        .attr('dy', '0.32em')
        .text(d => d)
        .on('mouseover', (d, i) => {
